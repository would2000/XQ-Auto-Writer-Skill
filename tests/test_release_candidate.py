...
CONTRACT_PATH = PROJECT_ROOT / "release" / "rc-interface-v3.json"
...
class ReleaseCandidateTests(unittest.TestCase):
    ...
    def test_target_must_be_the_next_minor_without_changing_version(self) -> None:
        def mutate(contract) -> None:
            contract["target_release_version"] = "2.0.0"

        result = rc.validate_release_candidate(PROJECT_ROOT, self._write_contract(mutate))
        self.assertFalse(result["ready"])
        self.assertIn("version_contract_mismatch", [item["code"] for item in result["errors"]])

    def test_minor_contract_requires_explicit_transition_marker(self) -> None:
        def mutate(contract) -> None:
            contract["release_transition"] = "major"
