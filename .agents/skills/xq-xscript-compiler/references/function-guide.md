...
涉及日期區間報酬、變數序列、`SetTotalBar`、指定頻率 `SetBarBack`、`OutputField`／`GetFieldDate` 或 XQ 18.01 函數參數能力時，另讀 [xspractice-learning-guide.md](xspractice-learning-guide.md)。該文件是 2016–2025 官方文章的文件蒸餾；本指南的目前 XSHelp、編譯器與 XQ 3.19.03 證據優先。
...
這些 caller 都傳入 `0`，因此可讀回同一個 `Close - Open` 值；指標畫出值、選股與警示把正值作為條件、自動交易只計算變數而不含 `SetPosition`、帳號或下單。產生器不開啟 XQ、不覆寫輸出，也不證明編譯或任何執行結果；仍須依本指南完成前置檢查、正確函數類型建檔與當次編譯。
...
- 舊版語法文章只提供概念背景：`SetTotalBar` 控制正式執行範圍，`SetBarBack` 準備指定來源頻率的引用深度，caller `[n]` 才是實際索引。三者及介面預載仍須使用獨立案例、較短足量控制組與唯一 marker 驗證，不能由舊文中的 bar 換算或別名推論目前結果。
