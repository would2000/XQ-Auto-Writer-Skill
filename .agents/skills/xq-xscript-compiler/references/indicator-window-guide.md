...
技術分析頁的交易訊號標記與分點指標文件模型另見 [XQ 進階應用官方課程蒸餾](advanced-learning-guide.md)。畫面標記不是 Plot 數值、XScript 執行證據或回測報告；不得為了驗證指標而開啟帳號明細、刪除委託或讀取私人交易資料。

2026-08-02 第十一階段在函數 `MyTrendScore` 與 caller `MyTrendScoreIndicator` 各自取得當次編譯成功後，改由唯一 `XS指標 > 自訂 > CODEX > MyTrendScoreIndicator` 加入臨時副圖，原生 Excel 匯出取得 99 列並完成原技術線圖頁復原。這是 Plot 存在與匯出流程證據；本案沒有指定獨立 expected column，因此不宣稱 99 列公式已逐列驗算。
...
3. 在 `新增副圖指標設定` 對話框中：
   ...
   - 搜尋後只接受唯一 `XS指標 > 自訂 > CODEX > <精確腳本名稱>` 的直接子文件；不得使用 `自訂` 根層或私人資料夾中的同名腳本。
