...
- 主要交易語法文章版本為 2021-03-17；自動交易錯誤代碼文章版本為 2024-12-10，於 2026-08-01 依使用者要求重新讀取。只保存自行改寫的狀態模型、執行規則、風險檢查、代碼原因、來源 URL 與版本限制，不保存官方正文。
...
- 策略雷達工作須讀取 [sensor-learning-guide.md](sensor-learning-guide.md)。該文件完整蒸餾 `lesson/sensor` 左側從「策略雷達基本操作」至「策略雷達與雷達回測的錯誤代碼說明」的 15 頁，涵蓋警示建立、洗價與觸發節流、基準商品、跨商品／跨頻率、雷達回測設定與報告、手機傳送、下單／平倉安全邊界及兩套錯誤碼。
- 15 頁文章日期介於 2016-01-11 至 2025-07-18，本次於 2026-07-24 依使用者明確要求逐頁讀取。只保存重新表述的操作契約、來源 URL／本地 ID、文章日期與版本限制；`body_text_stored` 維持 `false`。
- 「XS策略雷達高手分享篇」只蒸餾 `xq.com.tw` 上的目錄頁；沒有開啟或保存其連到 `xstrader` 的文章與程式碼。舊版入口、模組名稱、交易時段、回測模型與商品支援不得當成目前 XQ 的固定事實。
- 策略雷達下單、平倉、手機登入及帳號設定的文件知識不構成操作授權。Codex 仍不得登入／登出、操作實際帳號或送實單，只能在使用者授權且正面識別為 XQ 內建模擬帳號時測試。
- 雷達錯誤碼必須先區分即時策略雷達的 5000 系列與雷達回測代碼，且只能記錄 XQ 當次實際回報；本地 timeout、Windows 無回應或推測的網路問題不能代換成官方代碼。

## XS 選股課程

- 選股工作須讀取 [screener-learning-guide.md](screener-learning-guide.md)。該文件完整蒸餾 `lesson/filter` 左側從「選股因子分析：用數據說話，打造高勝率選股策略」至「用XS寫籌碼集中度的選股策略」的 13 頁，涵蓋選股中心、每日清單、欄位／腳本條件、自訂排行、`OutputField` 排序、因子分析、回溯、回測、題材型設計模式及選股錯誤碼。
- 13 頁文章日期介於 2016-01-11 至 2025-12-30，本次於 2026-07-24 依使用者明確要求逐頁讀取。只保存重新表述的操作契約、來源 URL／本地 ID、文章日期與版本限制；`body_text_stored` 維持 `false`，不保存文章股票名單或完整程式。
- 舊版選股中心入口、策略數量限制、資料更新、回測畫面與支援範圍不得當成目前 XQ 的固定事實；真實操作仍以 [screener-window-guide.md](screener-window-guide.md) 的 XQ 3.19.03 證據及現場讀回為準。
- 因子分析、排行、回溯與回測只提供歷史分析證據，不保證未來獲利。錯誤碼只能記錄 XQ 當次實際顯示；空集合、本地 timeout、Windows 無回應或推測的網路問題不得代換成官方代碼。

## XS 語法應用課程

- 涉及 XQ 18.01 版本能力、商品代碼／群組參數、編譯警告、`SetTotalBar`、指定頻率 `SetBarBack`、變數序列、`Print`、`OutputField`、`GetFieldDate` 或區間漲跌幅函數時，須讀取 [xspractice-learning-guide.md](xspractice-learning-guide.md)。
- 該文件完整蒸餾 `lesson/xspractice` 左側從「XQ 18.01 版本 XS 重點功能：美股盤前盤後、函數彈性與編輯器優化」至「計算區間漲跌幅的自訂函數」共 7 頁。文章日期介於 2016-03-09 至 2025-12-30，本次於 2026-07-24 依使用者明確要求逐頁讀取。
- 只保存重新表述的語法與資料契約、來源 URL／本地 ID、文章日期及版本限制；`body_text_stored` 維持 `false`，不保存正文、HTML、圖片或完整官方範例。
- 舊版安裝路徑、介面、支援腳本、函數別名、美股時段與委託限制不得當成目前固定事實。資料範圍也必須把預載、`SetTotalBar`、`SetBarBack` 與 `[n]` 分開驗證。

## XQ 進階應用課程

- 涉及經濟指標行事曆、自設畫面、美股全部時段、交易訊號標記、籌碼／券商／股權分析、籌碼選股、族群透視或分點指標時，須讀取 [advanced-learning-guide.md](advanced-learning-guide.md)。
- 該文件完整蒸餾 `lesson/advanced` 側欄的 17 篇內容頁，文章日期介於 2025-08-18 至 2026-05-04，本次於 2026-07-24 依使用者明確要求逐頁讀取；分類首頁只作範圍 metadata，不另算內容頁。
- 只保存自行重新表述的功能模型、操作契約、安全邊界、來源 URL／本地 ID、文章日期與版本限制；`body_text_stored` 維持 `false`，不保存正文、HTML、圖片、完整表格、完整官方範例或歷史個股／券商名單。族群文章另列的七個縮網址不屬側欄範圍，未展開或擷取。
- 舊版畫面、模組、資料欄位、分類與數值單位不得當成目前 XQ 的固定事實；自設頁面、自選券商及追蹤清單視為使用者私人設定。交易訊號標記頁中的帳號明細、刪單、全部刪單與下單描述只屬文件知識，不放寬憲法對帳號、委託與實單操作的禁止。
...
涉及指標繪圖設定視窗、Plot 序列樣式、顏色、座標軸或查價視窗時，另讀取 [plot-style-learning-guide.md](plot-style-learning-guide.md)。該文件只保存本機 UI 校正後的可泛化控制契約，不保存私人腳本正文或原始控制樹；繪圖數值與視覺樣式仍須分開驗證。
