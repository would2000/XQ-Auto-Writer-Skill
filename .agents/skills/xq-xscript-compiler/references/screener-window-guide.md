...
2026-08-02 第十一階段以 `MyBreakoutStrength` → `MyBreakoutStrengthScreener` 的當次編譯證據完成公開系統範圍代表回測：成功商品 50、失敗 0、交易 32。執行超過一分鐘時，精確 XQ 視窗顯示正常「選股執行中」，未補送 Start；唯一新增報告的 caller marker 相符，清理後 recovery-status 為 `safe_to_start` 且無 checkpoint。這不是策略績效證明。
...
## 從 XScript 加入選股並整理至 `自訂/CODEX`

2026-07-26 在 XQ 3.19.03 實測完成 `MyBreakoutStrengthScreener` 的非執行整理流程：XScript 編輯器的具名按鈕 `加入選股` 開啟「新增選股策略」，讀回預填的策略名稱、台股／不指定／普通股全部(系統)與唯一已加入腳本後，以 `完成(K)` 建立基本策略。此步只建立策略，不按「啟動」或執行選股。

- 選股中心左側目錄為 `SysTreeView32`、control ID `17580`。只可在唯一的 `自訂` 根節點下建立或使用直接子節點 `CODEX`；其他既有子節點一律保持唯讀。
- 若 `CODEX` 不存在，先以 UIA、MSAA、Win32 與正式鍵盤選單探測；本版自繪根節點沒有可用的正式鍵盤選單。在使用者授權的座標例外下，對目前、已讀回的唯一 `自訂` TreeItem 單步叫出選單，再以語意 MenuItem `新增資料夾`（ID `17647`）執行。XQ 直接產生樹內 Edit，不會另開命名對話框；輸入 `CODEX` 後按 Enter，必須讀回它是 `自訂` 的直接子節點。
- 以策略搜尋 `17786` 取得唯一目標列後，策略清單的右鍵選單提供語意命令 `移動...`（ID `20204`）。移動對話框標題為「移動選股策略」；選取唯一 `CODEX` TreeItem，並確認目錄文字 `17660` 為 `[自訂]CODEX`，再按 `完成(K)`（ID `1`）。
- 收尾時再次選取 `自訂/CODEX`，讀回中央唯一策略列的「自訂」欄位為 `CODEX`、名稱為目標策略，並讀回右側連結的選股腳本。此驗證只確認策略的分類位置，不代表策略已執行、條件有效或有投資績效。

## 指定歷史日期的選股結果

2026-07-26 在 XQ 3.19.03 實測結果區的日期控制。結果窗左下日期列的上一日按鈕為 `20755`、下一日為 `20756`、日曆按鈕為 `20757`；目前日期與「最新」標示是同列 `Static`，不可僅依畫面位置推測。

- 按 `20757` 會開啟獨立頂層 `#32770` 視窗「選擇指定日期」。其日曆是原生 `SysMonthCal32`、control ID `20714`；確認／取消分別是 Button ID `1`／`2`。
- 原生日曆可讀回目前選取日，且 `set_current_date(year, month, day_of_week, day)` 可設定日期；`day_of_week` 採 Windows `SYSTEMTIME`（星期日為 `0`）。已用 `2026-07-24`（星期五 `5`）進行同日期設定及讀回，並以取消關閉，未執行策略。
- 未來要依使用者指定日期取得結果時，先取得明確 `YYYY-MM-DD`、確認唯一目標策略仍位於 `自訂/CODEX`，並讀回日曆設定後的完整日期，才可按 `確定(K)`。接著必須等候 XQ 的啟動／停止狀態完整復原，分別擷取符合條件與執行錯誤結果，並回報策略、範圍、篩選日期、執行時間、命中數與錯誤數。
- 取消、關閉日曆或只設定日期並不構成歷史選股執行證據。未取得使用者明確日期時，不得自行選擇過去日期或啟動策略；任何結果也只代表該策略、範圍、日期與 XQ 版本下的當次輸出，不代表投資建議或績效保證。

2026-07-26 以使用者指定、已驗證位於 `自訂/CODEX` 的策略完成一次歷史日期實測。原生日曆 `MCM_SETCURSEL` 雖能改變控制項讀回值，卻沒有觸發 XQ 的結果日期事件；日曆日期格也沒有 UIA／MSAA 子項，且在此版本有邏輯座標與實際像素不一致的風險。因此不得把程式化設定或未經主結果日期讀回的日期格點選當成執行證據。

- 對相鄰歷史日期，可優先使用正式「上一日」按鈕 `20755`。每次只按一次，等待 XQ 顯示「選股正在執行中」或讀回啟動停用／停止啟用，並在啟動重新啟用、停止停用後才允許下一次日期切換；XQ 會忽略執行中的後續日期輸入。
- 實測顯示此方式會切換主結果日期並重新執行策略。完成後必須從結果區讀回目標日期，再檢查 ComboBox `20665` 的 `符合條件商品` 與 `執行錯誤的商品`，最後恢復呼叫前的結果類型。不能只以顯示中的舊表格、日期箭頭按下或停止按鈕暫時出現當作成功。
