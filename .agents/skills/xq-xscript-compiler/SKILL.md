...
When learning or calibrating an XScript custom folder, read [references/windows-calibration.md](references/windows-calibration.md) and use `scripts/xq_folder_observer.py` while the user performs the flow. Treat only an exact `CODEX (<count>)` direct child of the exact `自訂 (<count>)` root as `自訂/CODEX/`. The XQ 3.19.03 context menu was a standard `#32768`, not `XTPPopupBar`; do not restrict transient detection to XTP controls. Keep creation-dialog controls unverified until one complete creation is observed, and never promote a current-category observation to all five script types.

Before any XScript creation, require a calibrated feature-specific CODEX selector and exact `自訂/CODEX/` location readback. If XQ exposes no stable selector or the folder is absent, stop before opening the new-script dialog; never fall back to `自訂/`, geometry-only folder guessing, a coordinate-based action, or a private folder. Control-level activation is allowed only after exact control identification and must not receive a `coords` argument.
